...
\begin{tikzpicture}
	...
	\node[draw, align=center] (VAR) at ([xshift=-30mm]0,0) {
		panel of \\
		\textbf{reduced-form VAR~\eqref{eq:VAR}} };
	...
	\node[draw, align=left, text width=0.9\textwidth, shape=rectangle split, rectangle split parts=3] (B) at (0,-5) {
		\textbf{Decomposition} \\
		$ \text{Cov}[\boldsymbol{u}_{it}] = \mathsf{B}_i^{\ } \mathsf{B}_i' =: \Sigma_{u,i} $ for structural shocks $ \ \boldsymbol{\epsilon}_{it} = \mathsf{B}^{-1}_i \boldsymbol{u}_{it} $ \\
		
		\nodepart{two}
		$ \ \Omega_{u,i} := \mathsf{B}_{\sigma i}^{-1} \Sigma_{u,i} \mathsf{B}_{\sigma i}^{-1} = \textsf{B}_{\Omega i}^{\ } \textsf{B}_{\Omega i}' $ \\
		...
		$ \ \boldsymbol{\epsilon_\theta}_{it} = \textsf{B}_i(\boldsymbol{\theta})^{-1} \boldsymbol{u}_{it} = \mathsf{Q} \left( \boldsymbol{\theta} \right)^{-1} \textsf{B}_{\Omega i}^{-1} \mathsf{B}_{\sigma i}^{-1} \boldsymbol{u}_{it} = \mathsf{Q} \left( \boldsymbol{\theta} \right)' \boldsymbol{\epsilon_0}_{it} $};
